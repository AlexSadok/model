...
if (elgg_instanceof($page_owner, 'user')) {
	$url_segment = 'model/archive/' . $page_owner->username;
} else {
	$url_segment = 'model/group/' . $page_owner->getGUID() . '/archive';
}

// This is a limitation of the URL schema.
if ($page_owner && $vars['page'] != 'friends') {
	$dates = get_entity_dates('object', 'model', $page_owner->getGUID());

	if ($dates) {
		$dates = array_reverse($dates);
		
		$title = elgg_echo('model:archives');
		$content = '<ul class="model-archives">';
